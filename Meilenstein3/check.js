...
function checkjava() {
    ...
    if (btitel.length < 1) {
        if (bool == false) {
            document.getElementById("btitel").focus();
            bool = true;
        }
        document.getElementById("btitel").style.borderColor = "red";
        exception();
    }
    ...
    var vorname = document.send.vorname.value;
    if (!(vorname.match(/[a-zA-Z]+$/))) {
        if (bool == false) {
            document.send.vorname.focus();
            bool = true;
        }
        document.send.vorname.style.borderColor = "red";
        exception();
    }
